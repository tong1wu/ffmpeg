...
static int d3d12va_create_decoder_heap(AVCodecContext *avctx, int max_dpb_count)
{
    D3D12VADecodeContext   *ctx        = D3D12VA_DECODE_CONTEXT(avctx);
    AVHWFramesContext      *frames_ctx = D3D12VA_FRAMES_CONTEXT(avctx);
    ...
    D3D12_VIDEO_DECODER_HEAP_DESC desc = {
        ...
        .MaxDecodePictureBufferCount = max_dpb_count + 1 /* Current picture */,
    };
...
int ff_d3d12va_decode_init(AVCodecContext *avctx, int max_dpb_count)
{
    int ret;
    UINT bitstream_size;
    ...
    ret = d3d12va_create_decoder_heap(avctx, max_dpb_count);
    if (ret < 0)
        goto fail;

    ctx->max_num_res = frames_ctx->initial_pool_size;

    bitstream_size = ff_d3d12va_get_suitable_max_bitstream_size(avctx);
    ctx->buffers = av_calloc(sizeof(ID3D12Resource *),  ctx->max_num_res);
    for (int i = 0; i < ctx->max_num_res; i++) {
        ret = d3d12va_create_buffer(avctx, bitstream_size, &ctx->buffers[i]);
        if (ret < 0)
            goto fail;
    }

    ctx->ref_resources = av_calloc(sizeof(ID3D12Resource *), ctx->max_num_res);
    if (!ctx->ref_resources)
        return AVERROR(ENOMEM);

    ctx->ref_subresources = av_calloc(sizeof(UINT), ctx->max_num_res);
    if (!ctx->ref_subresources)
        return AVERROR(ENOMEM);

    ctx->index_mapped = av_calloc(sizeof(int), ctx->max_num_res);
    if (!ctx->index_mapped)
        return AVERROR(ENOMEM);

    ctx->allocator_queue = av_fifo_alloc2(frames_ctx->initial_pool_size, sizeof(CommandAllocator), AV_FIFO_FLAG_AUTO_GROW);
...
int ff_d3d12va_decode_uninit(AVCodecContext *avctx)
{
    ...
    av_freep(&ctx->index_mapped);

    frames_ctx = D3D12VA_FRAMES_CONTEXT(avctx);
    for (i = 0; i < ctx->max_num_res; i++)
...
void ff_d3d12va_refresh_ref_frame_list(AVCodecContext *avctx, UCHAR *curr, UCHAR *ref_frame_list, size_t num_ref_frame_list, int num_ref_frames)
{
    D3D12VADecodeContext   *ctx          = D3D12VA_DECODE_CONTEXT(avctx);
    AVHWFramesContext      *frames_ctx   = D3D12VA_FRAMES_CONTEXT(avctx);
    AVD3D12VAFramesContext *frames_hwctx = frames_ctx->hwctx;
    ...
    for (i = 0; i < num_ref_frame_list; i++) {
        if ((ref_frame_list[i] & 0x7f) != 0x7f) {
            if (ref_frame_list[i] >= ctx->max_num_res) {
                av_log(avctx, AV_LOG_WARNING, "The surface index `%d` is out of range(%d)!",
                    ref_frame_list[i], ctx->max_num_res);
                continue;
            }
            index = ctx->index_mapped[ref_frame_list[i]];
            ctx->ref_resources[index] = frames_hwctx->texture_infos[ref_frame_list[i]].texture;
            ref_frame_list[i] &= 0x80;
            ref_frame_list[i] |= index;
            used_mask         |= 1 << index;
        }
    }
                              ...
                              int(*update_input_arguments)(AVCodecContext *, D3D12_VIDEO_DECODE_INPUT_STREAM_ARGUMENTS *, ID3D12Resource *))
{
    ...
    memset(ctx->ref_resources, 0, sizeof(ID3D12Resource *) * ctx->max_num_res);
    memset(ctx->ref_subresources, 0, sizeof(UINT) * ctx->max_num_res);

    input_args.ReferenceFrames.NumTexture2Ds = ctx->max_num_res;
    ...
    (void)d3d12va_update_reference_frames_state(avctx, &barriers[1], index, D3D12_RESOURCE_STATE_VIDEO_DECODE_READ, D3D12_RESOURCE_STATE_COMMON);
