                                ...
                                HWBaseEncodePicture *base_pic)
{
    ...
    pic->encoded_metadata_ref = av_buffer_pool_get(ctx->encoded_metadata_pool);
    if (!pic->encoded_metadata_ref) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
    pic->encoded_metadata = (ID3D12Resource *)pic->encoded_metadata_ref->data;

    pic->resolved_metadata_ref = av_buffer_pool_get(ctx->resolved_metadata_pool);
    if (!pic->resolved_metadata_ref) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
    pic->resolved_metadata = (ID3D12Resource *)pic->resolved_metadata_ref->data;
...
#define TRANSITION_BARRIER(res, before, after)                      \
    ...
    D3D12_RESOURCE_BARRIER Metadatabarriers[1];
    Metadatabarriers[0] = TRANSITION_BARRIER(pic->encoded_metadata,
                                             D3D12_RESOURCE_STATE_VIDEO_ENCODE_WRITE,
                                             D3D12_RESOURCE_STATE_VIDEO_ENCODE_READ);

    ID3D12VideoEncodeCommandList2_ResourceBarrier(cmd_list, 1, Metadatabarriers);
...
fail:
    ...
    av_buffer_unref(&pic->encoded_metadata_ref);
    av_buffer_unref(&pic->resolved_metadata_ref);
    return err;
}
...
static D3D12VAEncodePicture *d3d12va_encode_alloc(AVCodecContext *avctx)
{
    D3D12VAEncodeContext *ctx = avctx->priv_data;
    D3D12VAEncodePicture *pic;
                                 ...
                                 HWBaseEncodePicture *base_pic, AVPacket *pkt)
{
    ...
    hw_base_encode_set_output_property(avctx, base_pic, pkt_ptr,
                                       ctx->codec->flags & FLAG_TIMESTAMP_NO_DELAY);

    return 0;
}

static int d3d12va_encode_check_frame(AVCodecContext *avctx,
                                      const AVFrame *frame)
{
    D3D12VAEncodeContext *ctx = avctx->priv_data;
...
static int d3d12va_encode_send_frame(AVCodecContext *avctx, AVFrame *frame)
{
    HWBaseEncodeContext *base_ctx = avctx->priv_data;
    D3D12VAEncodeContext     *ctx = avctx->priv_data;
    D3D12VAEncodePicture     *pic;
    ...
    if (frame) {
        ...
        err = d3d12va_encode_check_frame(avctx, frame);
        if (err < 0)
            return err;

        pic = d3d12va_encode_alloc(avctx);
        if (!pic)
            return AVERROR(ENOMEM);
...
fail:
    d3d12va_encode_free(avctx, base_pic);
    return err;
}
...
static const D3D12VAEncodeRCMode d3d12va_encode_rc_modes[] = {
    ...
    //                                                                  | Maxrate | HRD/VBV
    { 0 }, //                                                           |    |    |    |
    { RC_MODE_CQP,  "CQP",  D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_CQP,  0,   0,   1,   0 },
    { RC_MODE_CBR,  "CBR",  D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_CBR,  1,   0,   0,   1 },
    { RC_MODE_VBR,  "VBR",  D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_VBR,  1,   1,   0,   1 },
    { RC_MODE_ICQ,  "ICQ",  0 },
    { RC_MODE_QVBR, "QVBR", D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_QVBR, 1,   1,   1,   1 },
    { RC_MODE_AVBR, "AVBR", 0 },
};
...
rc_mode_found:
    ...
    if (rc_bits_per_second          > UINT32_MAX ||
        ...
        av_log(avctx, AV_LOG_ERROR, "RC parameters of 2^32 or "
               "greater are not supported by D3D12VA.\n");
        return AVERROR(EINVAL);
    }
    ...
    switch (rc_mode->mode) {
        ...
        case RC_MODE_CBR:
            ...
            cbr_ctl->TargetBitRate      = rc_bits_per_second;
        ...
        case RC_MODE_VBR:
            ...
            vbr_ctl->TargetAvgBitRate   = rc_bits_per_second * (rc_target_percentage / 100.0);
            vbr_ctl->PeakBitRate        = rc_bits_per_second;
        ...
        case RC_MODE_QVBR:
            ...
            qvbr_ctl->TargetAvgBitRate = rc_bits_per_second * (rc_target_percentage / 100);
            qvbr_ctl->PeakBitRate      = rc_bits_per_second;
...
static int d3d12va_create_encoder(AVCodecContext *avctx)
{
    HWBaseEncodeContext    *base_ctx     = avctx->priv_data;
    D3D12VAEncodeContext   *ctx          = avctx->priv_data;
    AVD3D12VADeviceContext *hwctx        = ctx->hwctx;
    AVD3D12VAFramesContext *frames_hwctx = base_ctx->input_frames->hwctx;
    HRESULT hr;
...
static AVBufferRef *d3d12va_encode_alloc_encoded_metadata_buffer(void *opaque, size_t size)
{
    AVCodecContext     *avctx = opaque;
    D3D12VAEncodeContext *ctx = avctx->priv_data;
    ID3D12Resource *pResource = NULL;
    ...
    hr = ID3D12Device_CreateCommittedResource(ctx->hwctx->device, &heap_props, D3D12_HEAP_FLAG_NONE,
                                              ...
                                              &IID_ID3D12Resource, (void **)&pResource);
    if (FAILED(hr)) {
...
static AVBufferRef *d3d12va_encode_alloc_resolved_metadata_buffer(void *opaque, size_t size)
{
    AVCodecContext     *avctx = opaque;
    D3D12VAEncodeContext *ctx = avctx->priv_data;
    ID3D12Resource *pResource = NULL;
    HRESULT hr;
    AVBufferRef *ref;
    int width = sizeof(D3D12_VIDEO_ENCODER_OUTPUT_METADATA) + sizeof(D3D12_VIDEO_ENCODER_FRAME_SUBREGION_METADATA); //hardcode
    D3D12_HEAP_PROPERTIES heap_props;
    D3D12_HEAP_TYPE heap_type = D3D12_HEAP_TYPE_READBACK;
    ...
    hr = ID3D12Device_CreateCommittedResource(ctx->hwctx->device, &heap_props, D3D12_HEAP_FLAG_NONE,
                                              ...
                                              &IID_ID3D12Resource, (void **)&pResource);
    if (FAILED(hr)) {
        av_log(avctx, AV_LOG_ERROR, "Failed to create resolved metadata buffer.\n");
        return NULL;
    }
...
static int d3d12va_encode_prepare_output_buffers(AVCodecContext *avctx)
{
    ...
    ctx->encoded_metadata_pool = av_buffer_pool_init2(sizeof(ID3D12Resource *), avctx,
                                                      &d3d12va_encode_alloc_encoded_metadata_buffer,
                                                      NULL);
    if (!ctx->encoded_metadata_pool)
        return AVERROR(ENOMEM);

    ctx->resolved_metadata_pool = av_buffer_pool_init2(sizeof(ID3D12Resource *), avctx,
                                                       &d3d12va_encode_alloc_resolved_metadata_buffer,
                                                       NULL);
    if (!ctx->resolved_metadata_pool)
        return AVERROR(ENOMEM);
...
static int d3d12va_encode_create_recon_frames(AVCodecContext *avctx)
{
    ...
    constraints = av_hwdevice_get_hwframe_constraints(base_ctx->device_ref, NULL);
    if (!constraints) {
        err = AVERROR(ENOMEM);
        goto fail;
    }
...
static const HWEncodeType d3d12va_type = {
    .send_frame = &d3d12va_encode_send_frame,
...
int ff_d3d12va_encode_init(AVCodecContext *avctx)
{
    ...
    base_ctx->has_sync_buffer_func = 1;
...
int ff_d3d12va_encode_close(AVCodecContext *avctx)
{
    ...
    av_buffer_pool_uninit(&ctx->encoded_metadata_pool);
    av_buffer_pool_uninit(&ctx->resolved_metadata_pool);
