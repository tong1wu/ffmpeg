...
static int update_input_arguments(AVCodecContext *avctx, D3D12_VIDEO_DECODE_INPUT_STREAM_ARGUMENTS *input_args, ID3D12Resource *buffer)
{
    ...
    ctx->ref_resources[0] = frames_hwctx->texture_infos[index].texture;

    for (int i = 0; i < FF_ARRAY_ELEMS(ctx_pic->pp.RefFrameMapTextureIndex); i++) {
        index = ctx_pic->pp.RefFrameMapTextureIndex[i];
        if (index != 0xFF)
            ctx->ref_resources[i + 1] = frames_hwctx->texture_infos[index].texture;
    }
...
static int d3d12va_av1_decode_init(AVCodecContext *avctx)
{
    ...
    ret = ff_d3d12va_decode_init(avctx, sizeof(ctx_pic->pp.RefFrameMapTextureIndex) + 1);
    if (ret < 0)
        return ret;
