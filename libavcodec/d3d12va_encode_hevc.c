...
#define D3D_PROFILE_DESC(name) { sizeof(D3D12_VIDEO_ENCODER_PROFILE_HEVC), &profile_ ## name }
static const D3D12VAEncodeProfile d3d12va_encode_hevc_profiles[] = {
    { FF_PROFILE_HEVC_MAIN,     8, 3, 1, 1, D3D_PROFILE_DESC(main)   },
    { FF_PROFILE_HEVC_MAIN_10, 10, 3, 1, 1, D3D_PROFILE_DESC(main10) },
    { FF_PROFILE_UNKNOWN }
};
...
static int d3d12va_encode_hevc_configure(AVCodecContext *avctx)
{
    ...
    if (ctx->rc.Mode == D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_CQP) {
        ...
        cqp_ctl->ConstantQP_InterPredictedFrame_BiDirectionalRef = fixed_qp_p;
        cqp_ctl->ConstantQP_InterPredictedFrame_PrevRefOnly      = fixed_qp_b;
