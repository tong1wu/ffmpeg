...
static int update_input_arguments(AVCodecContext *avctx, D3D12_VIDEO_DECODE_INPUT_STREAM_ARGUMENTS *input_args, ID3D12Resource *buffer)
{
    ...
    input_args->ReferenceFrames.NumTexture2Ds = ctx_pic->pp.num_ref_frames + 1;
    ff_d3d12va_refresh_ref_frame_list(avctx, &ctx_pic->pp.CurrPic, (UCHAR *)ctx_pic->pp.RefFrameList,
        FF_ARRAY_ELEMS(ctx_pic->pp.RefFrameList), ctx_pic->pp.num_ref_frames);
...
static int d3d12va_h264_decode_init(AVCodecContext *avctx)
{
    ...
    return ff_d3d12va_decode_init(avctx, FF_ARRAY_ELEMS(ctx_pic->pp.RefFrameList));
}
