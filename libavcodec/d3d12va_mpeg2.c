...
#define REF_RESOURCE(i,index) if (index != INVALID_REF) { \
    ctx->ref_resources[i] = frames_hwctx->texture_infos[index].texture; \
}
...
static int update_input_arguments(AVCodecContext *avctx, D3D12_VIDEO_DECODE_INPUT_STREAM_ARGUMENTS *input_args, ID3D12Resource *buffer)
{
    ...
    REF_RESOURCE(0, ctx_pic->pp.wDecodedPictureIndex    )
    REF_RESOURCE(1, ctx_pic->pp.wForwardRefPictureIndex )
    REF_RESOURCE(2, ctx_pic->pp.wBackwardRefPictureIndex)
...
static int d3d12va_mpeg2_decode_init(AVCodecContext *avctx)
{
    ...
    return ff_d3d12va_decode_init(avctx, 3);
}
